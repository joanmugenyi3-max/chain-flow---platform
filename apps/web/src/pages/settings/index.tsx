...
function SectionCard({ children, style }: { children: React.ReactNode; style?: React.CSSProperties }) {
  return (
    <div style={{
      ...
      boxShadow: colors.shadow, marginBottom: 20, ...style,
    }}>
      {children}
    </div>
  );
...
function SectionTitle({ children, action }: { children: React.ReactNode; action?: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
      <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: colors.slate900, fontFamily: font.sans, letterSpacing: '-0.3px' }}>
        {children}
      </h2>
      {action}
    </div>
...
function FieldRow({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '160px 1fr', alignItems: 'center', padding: '10px 0', borderBottom: `1px solid ${colors.border}` }}>
      <span style={{ fontSize: 13, color: colors.slate500, fontFamily: font.sans, fontWeight: 500 }}>{label}</span>
      <span style={{ fontSize: 14, color: colors.slate800, fontFamily: font.sans, background: colors.slate50, border: `1px solid ${colors.border}`, borderRadius: radius.sm, padding: '6px 12px', maxWidth: 340 }}>
        {value}
      </span>
    </div>
  );
      ...
      <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <SectionCard>
            <SectionTitle
              action={
                <button
                  onClick={() => {}}
                  style={{
                    background: 'transparent', color: colors.slate600,
            ...
            >
              {t('settingsPage.sectionOrganization')}
            </SectionTitle>

            <div>
              <FieldRow label={t('settingsPage.fieldOrgName')}  value={tenantName} />
              <FieldRow label={t('settingsPage.fieldIndustry')} value="Mining" />
              <FieldRow label={t('settingsPage.fieldCurrency')} value="USD" />
              <div style={{ display: 'grid', gridTemplateColumns: '160px 1fr', alignItems: 'center', padding: '10px 0' }}>
                <span style={{ fontSize: 13, color: colors.slate500, fontFamily: font.sans, fontWeight: 500 }}>
                  {t('settingsPage.fieldTimezone')}
                </span>
                <span style={{ fontSize: 14, color: colors.slate800, fontFamily: font.sans, background: colors.slate50, border: `1px solid ${colors.border}`, borderRadius: radius.sm, padding: '6px 12px', maxWidth: 340 }}>
          ...
          {/* 2. Current Plan */}
          <SectionCard>
            <SectionTitle>{t('settingsPage.sectionCurrentPlan')}</SectionTitle>

            <div style={{
              display: 'flex', alignItems: 'center', gap: 20, padding: '20px 24px',
              background: planColor + '0d', border: `2px solid ${planColor}40`,
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                  <span style={{ fontSize: 18, fontWeight: 700, color: planColor, fontFamily: font.sans, letterSpacing: '-0.3px' }}>
                    {planLabel}
                  </span>
                  <span style={{
                    ...
                    textTransform: 'uppercase', letterSpacing: '0.5px', fontFamily: font.sans,
                  }}>
                    {t('settingsPage.activeLabel')}
                  </span>
                </div>
                <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 13, color: colors.slate500, fontFamily: font.sans }}>{t('settingsPage.monthlyBilling')}</span>
                  <span style={{ fontSize: 13, color: colors.slate500, fontFamily: font.sans }}>{t('settingsPage.nextRenewal')}</span>
                </div>
              </div>
              ...
              <Link
                ...
                style={{
                  ...
                  padding: '8px 16px', background: planColor, color: colors.white,
                  borderRadius: radius.md, textDecoration: 'none',
                  fontSize: 13, fontWeight: 600, fontFamily: font.sans, flexShrink: 0,
                }}
              >
                {t('settingsPage.manageSub')}
              </Link>
            </div>
          </SectionCard>
          ...
          <div style={{
            ...
            borderRadius: radius.lg, padding: '24px 28px', boxShadow: colors.shadow,
          }}>
            <SectionTitle>{t('settingsPage.sectionDangerZone')}</SectionTitle>

            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
              <div>
                <div style={{ fontSize: 14, fontWeight: 600, color: colors.slate800, fontFamily: font.sans, marginBottom: 2 }}>
                  {t('settingsPage.resetDemoTitle')}
                </div>
                <div style={{ fontSize: 13, color: colors.slate500, fontFamily: font.sans }}>
                  {t('settingsPage.resetDemoDesc')}
                </div>
              </div>
              <button
                ...
                style={{
                  ...
                  border: `1.5px solid ${colors.warning}`, borderRadius: radius.md,
                  padding: '8px 18px', fontSize: 13, fontWeight: 600,
                  cursor: 'pointer', fontFamily: font.sans, flexShrink: 0,
                }}
              >
                {t('settingsPage.resetDemoBtn')}
              </button>
            </div>
